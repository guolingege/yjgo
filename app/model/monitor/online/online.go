...
//修改页面请求参数
type EditReq struct {
	SessionId      string      `p:"sessionId" v:"required#主键ID不能为空"`
	LoginName      string      `p:"loginName" v:"required#登录账号不能为空"`
	DeptName       string      `p:"deptName" v:"required#部门名称不能为空"`
	Ipaddr         string      `p:"ipaddr" `
	LoginLocation  string      `p:"loginLocation" `
...
func SelectListExport(param *SelectPageReq) (gdb.Result, error) {
	...
	model = model.Fields(" t.sessionId ,t.login_name ,t.dept_name ,t.ipaddr ,t.login_location ,t.browser ,t.os ,t.status ,t.start_timestamp ,t.last_access_time ,t.expire_time")
